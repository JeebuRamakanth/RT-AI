...
- **Suite**: 110 tests across 11 files (87 from Steps 02–03, 23 Asset Vault domain tests in Step 05) covering language/style detection (+ extended metadata + script registry), request/response creation, provider+registry+router, conversation lifecycle, error handling + stack-trace sanitization, cancellation, full Home→AI Core integration, emoji intelligence (+ tasteful application + budgets), conversation intelligence (intent, topic, continuity, preferences), mixed-language/style/emoji behaviour through the orchestrator, naming-normalization guards, and the Asset Vault (registration, trash/restore/permanent delete, favorites, archive, tags, filters, sorting, search, collections, versions, binary storage, conversation-independence, events, type/format helpers, counts).

## Asset Vault (Step 05)
A permanent, conversation-independent multimodal library at `/assets` (nav label "Asset Vault", `status: "available"`). **CHAT DELETE ≠ ASSET DELETE** and vice versa — both directions test-guarded.
- **Domain** (`src/assets/`): `types.ts` (Asset with ownerId/type/mimeType/size/source/sourceRef{conversationId,conversationTitle,messageId,isConversationDeleted}/storageReference/tags/collectionIds/favorite/archived/deletedAt/version/parentAssetId/generationMetadata/uploadMetadata/status/contentHash; AssetCollection; discriminated `AssetTypeMeta` for image/video/audio/document/pdf/presentation/spreadsheet/code/other), `repository.ts` (`LocalAssetRepository` over the shared `KeyValueStore`, key prefixes `asset:`/`acol:`/`index:assets`, plus an `AssetEvent` pub-sub seam for a future notification center), `storage.ts` (`AssetStorage` — Blobs in a dedicated IndexedDB DB `rt-ai-assets`; `MemoryAssetStorage` fallback; bytes NEVER in React state/localStorage), `store.tsx` (`AssetStoreProvider` — repository + storage + revision bump; owner comes from `useConversationStore`), `index.ts` (barrel).
- **Hooks**: `src/hooks/useAssetVault.ts` (lists/filters/sort/search state — search runs against the repository, not rendered cards), `src/hooks/useAssetActions.ts` (upload, favorite, rename, archive, tags, collections, trash/restore/permanent delete — permanent delete removes metadata AND bytes; `referenceFor()` is the future composer-attach seam).
- **UI** (`src/components/vault/`): `AssetVault.tsx` (orchestrator), `AssetGrid.tsx`, `AssetCard.tsx`, `AssetDetails.tsx` (dialog with provenance + tags + collections + actions), `AssetToolbar.tsx`, `AssetSearch.tsx`, `AssetFilters.tsx`, `AssetPreview.tsx` + `preview-utils.ts` (honest previews: real image/video/audio bytes, premium type card otherwise; non-component exports live in `preview-utils.ts` for fast-refresh), `AssetUpload.tsx` (hidden input + drag-drop in AssetVault).
- **Page**: `src/pages/AssetVaultPage.tsx` wraps its own `ConversationStoreProvider` + `AssetStoreProvider` + `ToastProvider` (same pattern as HistoryPage).
- Home keeps a single "Open Asset Vault →" shortcut in the RecentWork section header (Section `action` slot). No RT/ARTI-prefixed component names.
- Icons added to `src/components/icons/Icon.tsx`: star, star-filled, folder, upload, download, tag.
